...
```{r setup Exercise 2.3}
    ...
    knitr::opts_knit$set(root.dir = normalizePath("/home1/anmolsin/490_cluster")) 
```
...
my_var <- "My name is Anmol Singhal."
...
vector <- c("Basketball", "Soccer", "Football", "Hockey") 
...
numeric_vector <- c(13,45,19, 24, 57)
## great names:
...
birthday
typeof(birthday)
three_fav_colors <- c("Blue, Red, Green")
three_fav_colors
...
#One thing I notice when using the mode() function on a numeric vector, the output will
#just be "numeric". Below is a function I defined to get the mode of a vector.
find_mode_num_vector <- function(num_vector){
  unique_values <- unique(num_vector)
  unique_value_count <- table(num_vector)
  value_of_mode <- unique_values[which.max(unique_value_count)]
  return(value_of_mode)
}
find_mode_num_vector(list_of_numbers)
...
#Testing out ifelse() statement
ifelse("Cat"=="Cat","Meow", "Doesn't work")
...
if (7.5 %in% new_list) {
  ...
  print("D") #PREDICTION: "D" will print because starbucks is in new_list and !(9>10) is a true statement (! negates the false statement of 9>10)
  if (100 < 1 | "elephant" %in% new_list) {
    print("E") # PREDICTION: E" will print because this is an "or" statement and "elephant" being in new_list is satisfied
  }
